<?php
    $sqlProOrd = "SELECT orders.order_id FROM orders WHERE status = '0' OR status = '1';";
	$result = mysqli_query($conn, $sqlProOrd);
	$p = mysqli_num_rows($result);
?>
<?php 
    $rowPerPage = 5; //Số sản phẩm trên 1 trang.
    $sql_order = "SELECT * FROM orders WHERE status = '0' OR status = '1' ";
    ...
    $sql_pagination = "SELECT * FROM orders WHERE status = '0' OR status = '1' ORDER BY order_id DESC LIMIT $start,$rowPerPage";
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
	...
	<div id="toolbar" class="btn-group">
		<a href="index.php?page=order" style="background-color: red;" class="btn btn-success">
			<i class="glyphicon glyphicon-plus" style="background-color: red;"></i> Có <?php echo $p; ?> đơn chưa đuợc xử lí
		</a>
	</div>
	<div class="row">
		<div class="col-md-12">
							    			...
							    			<td>
                                                ...
                                                <!--                                         
                                                    if($row['status'] == 1) { 
                                                        ...
                                                        echo '<span class="label label-danger">hưa xử lý</span>';
                                                    } -->
                                            </td>
                                            <td class="form-group">
				    						    <a href="index.php?page=processed_order_detail&order_id=<?php echo $row['order_id']; ?>" class="btn btn-primary"><i class="glyphicon glyphicon-eye-open"></i></a>
